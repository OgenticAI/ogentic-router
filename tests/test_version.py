...
step that asserts ``ogentic_router.__version__ == <tag-without-v>``. If the tag
is ``v0.1.0`` but ``src/ogentic_router/__init__.py`` still has the previous
...
src/ogentic_router/__init__.py in the same commit."
"""
...
def test_version_matches_pyproject() -> None:
    ...
    Mirrors the assertion at ``release.yml`` smoke step so PR-time CI catches a
    mismatch instead of letting it land on PyPI first.
    """
    pyproject_path = Path(__file__).parent.parent / "pyproject.toml"
    pyproject = tomllib.loads(pyproject_path.read_text(encoding="utf-8"))
    expected = pyproject["project"]["version"]
    assert ogentic_router.__version__ == expected, (
        f"Version mismatch: ogentic_router.__version__ == "
        f"{ogentic_router.__version__!r} but pyproject.toml [project].version "
        f"== {expected!r}. Update src/ogentic_router/__init__.py to match — "
        f"see RELEASING.md."
    )
